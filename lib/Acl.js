...
ACL.prototype.getUserGroups = function(user_id, cb){
  ...
  var query = [
//  'MATCH p = allShortestPaths((user:User {user_id:{user}.user_id})-[rel:BELONGS_TO*]->(group:Usergroup ))',
//  'RETURN DISTINCT group, LENGTH(p) AS distance, RELATIONSHIPS(p)'
//  'RETURN group.name AS name, distance, [b IN belongs | b.since] AS since'
    'MATCH (user:User {user_id:{user}.user_id})-[rel:BELONGS_TO*]->(group:Usergroup )',
    'RETURN group.name AS name, length(rel) AS distance, REDUCE(since = 0, b IN rel | ',
    '                                                      CASE',
    '                                                        WHEN since >= b.since',
    '                                                        THEN since',
    '                                                        ELSE b.since',
...
*/
ACL.prototype.whatResources = function(groups, permissions, cb){

};

/*
*/
ACL.prototype.permittedResources = function(groups, permissions, cb){
