...
ACL.prototype.allowedPermissions = function(userId, resources, cb){
  ...
  /*
  MATCH (user:User {_id:{user}._id})-[belong:BELONGS_TO*]->(:Usergroup)-[permission:HAS_PERMISSION]->(res:Resource)
  WHERE res._id IN {resources}
  WITH res, permission
  RETURN res._id AS resource, permission.list AS permissions
  */
  var query = [
  'MATCH (user:'+ acl.settings.user_label +' {'+ acl.settings.user_id_key +':{user}.'+ acl.settings.user_id_key +'})-[belong:BELONGS_TO*]->(:Usergroup)-[permission:HAS_PERMISSION]->(res:Resource)',
