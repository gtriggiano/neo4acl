...
ACL.prototype._serializePermission = function(list) {
  var ret = {};
  ret[this.settings.has_permissions_list_key] = permissions;
  return ret;
};
...
ACL.prototype.removeResource = function(resourceId, cb){
  ...
  */
  var query = [
  'MATCH (res:'+ acl.settings.res_label+' {'+ acl.settings.rel_id_key +':{res}.'+ acl.settings.rel_id_key +')',
  'OPTIONAL MATCH (res)-[rels]-()',
  'DELETE rels, res'
  ];
  ...
  acl.db.query(query, parameters, function(err, result) {
    cb(err, result, query, parameters);
  });
...
ACL.prototype.allow = function(roles, resources, permissions, cb){
  ...
  MERGE (group0)-[rel0:HAS_PERMISSION]->(res0)
    ON CREATE SET rel.list = {rel}.list
    ON MATCH SET rel.list = DISTINCT (rel.list + {rel}.list)
  */
  var query = [
