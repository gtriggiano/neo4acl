...
/**
 * userHasAllRoles( userId, roles, function(err, bool, query, parameters) )
 *
 * Checks if the user has each of the passed roles
 *
 * @param {String|Number} User id.
 * @param {Function} Callback called when finished.
 * @return
...
ACL.prototype.userHasAllRoles = function(userId, roles, cb) {
  ...
  /*
  MATCH (user:User)-[:BELONGS_TO*]->(group:Usergroup)
  WHERE user._id = {user}._id} AND group.name IN {groups}
  RETURN count(DISTINCT group.name)
  */
  var query = [
  'MATCH (user:'+ acl.settings.user_label +')-[:'+ acl.settings.belongs +'*]->(group:'+ acl.settings.group_label +')',
  'WHERE user.'+ acl.settings.user_id_key +' = {user}.'+ acl.settings.user_id_key +' AND group.'+ acl.settings.group_name_key +' IN {groups}',
  'RETURN count(DISTINCT group.'+ acl.settings.group_name_key +') AS found'
  ];
  ...
  if (cb) {
    acl.db.query(query, parameters, function(err, result) {
      var bool = result.found < args.roles.length ? false : true;
      cb(err, bool, query, parameters);
    });
  }
...
ACL.prototype.userHasAnyRole = function(userId, roles, cb) {
  ...
  if (cb) {
    acl.db.query(userHasAllRoles.query, userHasAllRoles.parameters, function(err, result) {
      var bool = result.found > 0 ? true : false;
      cb(err, bool, userHasAllRoles.query, userHasAllRoles.parameters);
    });
  }
  
  promise.query = userHasAllRoles.query;
  promise.parameters = userHasAllRoles.parameters;
  return promise;
};
