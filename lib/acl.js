...
ACL.prototype.userRoles = function(userId, cb){
  ...
  /*
  MATCH (user:User {_id: {user}._id})-[:BELONGS_TO*]->(group:Usergroup)
  RETURN DISTINCT group.name
  */
  var query = [
  'MATCH (user:'+ acl.settings.user_label +' {'+ acl.settings.user_id_key +': {user}._id} )-[:'+ acl.settings.belongs +'*]->(group:'+ acl.settings.group_label +')',
  'RETURN COLLECT(DISTINCT group.'+ acl.settings.group_name_key +')'
  ].join('\n');
  
  acl.db.query(query, parameters, function(err, results) {
    cb(err, results[0], query, parameters);
  });
}; //OK

/**
 * addRoleParents(role, parents, function(err, done, query) )
 *
...
ACL.prototype.addRoleParents = function(role, parents, cb){
  var args = Args([
               ...
               [
                 {parents: Args.STRING | Args.Required},
                 {parents: Args.ARRAY    | Args.Required,
                  _check: function(parents) {
                    var flag = true;
  ...
  */
  var query = [
  'MATCH (group:'+ acl.settings.usergroup_label +' {group})'
  ];
  
  parents.forEach(function(group, i) {
    var id = acl.getIdentifiers(i);
    params[id.group] = acl.serializeUsergroup(group);
    cypher.push('MERGE (group)-[:'+ acl.settings.belongs_rel_name +']->(:'+ acl.settings.usergroup_label +' {'+ id.group +'})');
  });
  
  cypher.push('RETURN group');
  cypher = cypher.join('\n');
  
  acl.db.query(cypher, params, function(err, results) {
